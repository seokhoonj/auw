...
pste_string <- function(x, collapse = "|") paste0(x, collapse = collapse)
glue_string <- function(x, collapse = "|") paste0(unique(x[!is.na(x)]), collapse = collapse)
sort_string <- function(x, collapse = "|") paste0(sort(unique(x[!is.na(x)])), collapse = collapse)
...
pull_excl_term <- function(x, ignore.case = TRUE) {
  as.integer(gsub("[0-9].EXCL|EXCL|\\(.*?\\)", "", x, ignore.case = ignore.case, perl = TRUE))
}
